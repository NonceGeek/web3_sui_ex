defmodule Web3SuiEx.Aptos.AptosTest do
  @moduledoc false
  use ExUnit.Case, async: true

  setup_all do
     {:ok, rpc} = Web3SuiEx.Aptos.RPC.connect()
     priv = Web3SuiEx.Crypto.generate_priv()
     {:ok, account} = Web3SuiEx.Aptos.Account.from_private_key(priv)
    %{rpc: rpc, ac: account}
  end

  test "call_function", %{rpc: _rpc, ac: account} do
     f = %Web3SuiEx.Aptos.Types.Function{
   address: 0x1,
   is_entry: true,
   module: "coin",
   name: "transfer",
   ...
   _payload = Web3SuiEx.Aptos.call_function(f, ["0x1::aptos_coin::AptosCoin"], [account.address, 100,["abc", "456"]])
   assert _payload=%Web3SuiEx.Aptos.Types.TransactionPayload.EntryFunction{
